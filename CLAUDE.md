...
- `bun add [PACKAGE_NAME]`: Add a dependency
- `bun add -D [PACKAGE_NAME]`: Add a development dependency
- `bun remove [PACKAGE_NAME]`: Remove a dependency
- `bun run [SCRIPT_NAME]`: Run a script
- `bun test`: Run tests
...
- Import definitions directly from their files, not higher-level folders, e.g.,
  `import { useTheme } from '../../hooks/useTheme'` instead of
  `import { useTheme } from '../../hooks'`.
- Use and maintain a single `index.ts` at the top level of the source for all
  package exports.
