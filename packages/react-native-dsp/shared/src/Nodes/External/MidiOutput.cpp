...
dsp::MidiOutput::MidiOutput(std::shared_ptr<MidiBuffer> outputMessages,
                            Type type)
    : Consumer(type), outputMessages(outputMessages), previous(0.0) {}
...
void dsp::MidiOutput::setProcessFunction(
    ...
    for (size_t sample = 0; sample < getNumSamples(); ++sample) {
      for (size_t i = 0; i < getNumInputChannels(); ++i) {
        ...
        if (previous != current) {
          ...
          outputMessages->addEvent(
              MidiMessage::noteOff(channel, previousNote, uint8(0)), sample);
          outputMessages->addEvent(
              MidiMessage::noteOn(channel, currentNote, uint8(127)), sample);
          previous = current;
        }
      }
    }
...
std::function<void()> dsp::MidiOutput::processNoteOn(
  ...
  return [this, channel, noteSet]() {
    processImpulse(noteSet, [channel](uint8 note, Sample current) {
      return MidiMessage::noteOn(channel, note,
                                 static_cast<uint8>(unipolarToByte(current)));
    });
  };
}

std::function<void()> dsp::MidiOutput::processNoteOff(
    uint8 channel, std::unordered_set<uint8> noteSet) {
  return [this, channel, noteSet]() {
    processImpulse(noteSet, [channel](uint8 note) {
      return MidiMessage::noteOff(channel, note, uint8(0));
    });
  };
}

std::function<void()> dsp::MidiOutput::processNotePressure(
    uint8 channel, std::unordered_set<uint8> noteSet) {
  return [this, channel, noteSet]() {
...
std::function<void()> dsp::MidiOutput::processControlValue(
...
std::function<void()> dsp::MidiOutput::processProgram(uint8 channel) {
  return [this, channel]() {
    processContinuous([channel](Sample current) {
      return MidiMessage::programChange(channel, static_cast<uint8>(current));
    });
  };
}
...
std::function<void()> dsp::MidiOutput::processAllNotesOff(uint8 channel) {
  return [this, channel]() {
    processImpulse([channel]() { return MidiMessage::allNotesOff(channel); });
  };
}

std::function<void()> dsp::MidiOutput::processSongPositionInQuarterNotes() {
  return [this]() {
    processContinuous([](Sample current) {
      return MidiMessage::songPositionPointer(static_cast<uint8>(current * 4));
    });
  };
}
...
std::function<void()> dsp::MidiOutput::processClock() {
  return
      [this]() { processImpulse([]() { return MidiMessage::midiStart(); }); };
}
...
std::function<void()> dsp::MidiOutput::processContinue() {
  return [this]() {
    processImpulse([]() { return MidiMessage::midiContinue(); });
  };
}
...
void dsp::MidiOutput::processImpulse(
    std::unordered_set<uint8> itemSet,
    std::function<MidiMessage(uint8, Sample)> publish) {
  for (size_t sample = 0; sample < getNumSamples(); ++sample) {
    for (size_t channel = 0; channel < getNumInputChannels(); ++channel) {
      const Sample current =
          getInput()->getWrapper().getSample(channel, sample);
      if (current) {
...
void dsp::MidiOutput::processImpulse(
    std::unordered_set<uint8> itemSet,
    std::function<MidiMessage(uint8)> publish) {
  for (size_t sample = 0; sample < getNumSamples(); ++sample) {
    for (size_t channel = 0; channel < getNumInputChannels(); ++channel) {
...
void dsp::MidiOutput::processImpulse(
    std::function<MidiMessage(Sample)> publish) {
  for (size_t sample = 0; sample < getNumSamples(); ++sample) {
    for (size_t channel = 0; channel < getNumInputChannels(); ++channel) {
      const Sample current =
          getInput()->getWrapper().getSample(channel, sample);
...
void dsp::MidiOutput::processContinuous(
    std::unordered_set<uint8> itemSet,
    std::function<MidiMessage(uint8, Sample)> publish) {
  for (size_t sample = 0; sample < getNumSamples(); ++sample) {
    for (size_t channel = 0; channel < getNumInputChannels(); ++channel) {
