...
dsp::MidiInput::MidiInput(std::shared_ptr<MidiBuffer> inputMessages, Type type)
    : Producer(type), inputMessages(inputMessages), previous(0.0) {}
...
std::function<void()> dsp::MidiInput::processNote(
    ...
    processContinuous(
        [channel, noteSet](MidiMessage message) {
          return message.isNoteOn() &&
                 ...
                 (noteSet.empty() ||
                  noteSet.find(message.getNoteNumber()) != noteSet.end());
        },
        [](MidiMessage message) { return message.getNoteNumber(); });
  };
}

std::function<void()> dsp::MidiInput::processNoteOn(
    uint8 channel, std::unordered_set<uint8> noteSet) {
  return [this, channel, noteSet]() {
    processImpulse(
        [channel, noteSet](MidiMessage message) {
        ...
        [](MidiMessage message) {
          return byteToUnipolar(message.getVelocity());
        });
  };
}

std::function<void()> dsp::MidiInput::processNoteOff(
    uint8 channel, std::unordered_set<uint8> noteSet) {
  return [this, channel, noteSet]() {
    processImpulse(
        [channel, noteSet](MidiMessage message) {
...
std::function<void()> dsp::MidiInput::processNotePressure(
    uint8 channel, std::unordered_set<uint8> noteSet) {
  return [this, channel, noteSet]() {
    processContinuous(
        [channel, noteSet](MidiMessage message) {
          return (message.isAftertouch() || message.isNoteOn() ||
                  message.isNoteOff()) &&
                 (channel == 0 || channel == message.getChannel()) &&
                 (noteSet.empty() ||
                  noteSet.find(message.getNoteNumber()) != noteSet.end());
...
std::function<void()> dsp::MidiInput::processControl(
    ...
    processContinuous(
        [channel, controlSet](MidiMessage message) {
          return message.isController() &&
                 (channel == 0 || channel == message.getChannel()) &&
                 (controlSet.empty() ||
                  controlSet.find(message.getNoteNumber()) != controlSet.end());
        },
        [](MidiMessage message) { return message.getControllerNumber(); });
  };
}

std::function<void()> dsp::MidiInput::processControlValue(
    uint8 channel, std::unordered_set<uint8> controlSet) {
  return [this, channel, controlSet]() {
    processContinuous(
        [channel, controlSet](MidiMessage message) {
        ...
        [](MidiMessage message) {
          return byteToUnipolar(message.getControllerValue());
        });
  };
}
...
std::function<void()> dsp::MidiInput::processChannelPressure(uint8 channel) {
  return [this, channel]() {
    processContinuous(
        ...
        [](MidiMessage message) {
          return byteToUnipolar(message.getChannelPressureValue());
        });
  };
}
...
std::function<void()> dsp::MidiInput::processPitchWheel(uint8 channel) {
  return [this, channel]() {
    processContinuous(
        ...
        [](MidiMessage message) {
          return shortToBipolar(message.getPitchWheelValue());
        });
  };
}
...
std::function<void()> dsp::MidiInput::processSongPositionInQuarterNotes() {
  return [this]() {
    processContinuous(
        [](MidiMessage message) { return message.isSongPositionPointer(); },
        [](MidiMessage message) {
          return message.getSongPositionPointerMidiBeat() * 0.25;
        });
  };
}
...
void dsp::MidiInput::processImpulse(
    std::function<bool(MidiMessage)> conditional,
  ...
  for (const auto meta : *inputMessages) {
    ...
    for (size_t channel = 0; channel < getNumOutputChannels(); ++channel) {
      getOutput()->getWrapper().setSample(channel, meta.samplePosition,
                                          current);
    }
  }
}

void dsp::MidiInput::processContinuous(
    std::function<bool(MidiMessage)> conditional,
    std::function<Sample(MidiMessage)> capture) {
  ...
  for (const auto meta : *inputMessages) {
    ...
    if (conditional(message)) {
      getOutput()
          ->getWrapper()
          .getSampleRange(startSample, meta.samplePosition - startSample)
          .fill(previous);
  ...
  getOutput()
      ->getWrapper()
      .getSampleRange(startSample, getOutput()->getNumSamples() - startSample)
      .fill(previous);
}
