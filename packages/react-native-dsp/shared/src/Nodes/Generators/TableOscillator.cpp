...
dsp::TableOscillator::TableOscillator(Type type)
    : Producer(type),
      ...
      phaseInterpolation(std::make_shared<Input>(Type::INTEGER, Space::TIME,
                                                 Interpolation::MAX,
                                                 Interpolation::LINEAR)),
      positionInterpolation(std::make_shared<Input>(Type::INTEGER, Space::TIME,
                                                    Interpolation::MAX,
                                                    Interpolation::LINEAR)) {
...
std::shared_ptr<dsp::Input> dsp::TableOscillator::getPhaseInterpolation()
    const {
  return phaseInterpolation;
}

std::shared_ptr<dsp::Input> dsp::TableOscillator::getPositionInterpolation()
    const {
...
void dsp::TableOscillator::processNoLock() {
  if (tables.size() > 0) {
    ...
    for (size_t channel = 0; channel < getNumChannels(); ++channel) {
      ...
      Sample* phaseInterpolationChannel =
          getPhaseInterpolation()->getWrapper().getChannelPointer(channel);
      Sample* positionInterpolationChannel =
          getPositionInterpolation()->getWrapper().getChannelPointer(channel);
      Sample* outputChannel =
          getOutput()->getWrapper().getChannelPointer(channel);
      for (int sample = 0; sample < getNumSamples(); ++sample) {
        int phaseInterpolation =
            static_cast<int>(phaseInterpolationChannel[sample]);
        int positionInterpolation =
            static_cast<int>(positionInterpolationChannel[sample]);
        ...
        const Sample positionIndex =
            ...
            (tables.size() -
             (positionInterpolation == Interpolation::NONE ? 0 : 1));
            ...
            if (numChannels > 0) {
              ...
              switch (phaseInterpolation) {
                case Interpolation::NONE:
                  ...
                  break;
                case Interpolation::LINEAR:
                  samples[j] = linear(table, numSamples, index,
                                      tables[p]->getChannelValue(c));
                  break;
                case Interpolation::HERMITE:
                  samples[j] = hermite(table, numSamples, index,
                                       tables[p]->getChannelValue(c));
                  break;
              }
            } else {
        ...
        switch (positionInterpolation) {
          ...
          case Interpolation::LINEAR:
            outputChannel[sample] =
                linear(samples.data(), numPoints, positionIndex - indexBefore);
            break;
          case Interpolation::HERMITE:
            outputChannel[sample] =
                hermite(samples.data(), numPoints, positionIndex - indexBefore);
            break;
        }
      }
    }
