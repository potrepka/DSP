...
void dsp::MoorerOscillator::processNoLock() {
  for (int channel = 0; channel < getNumChannels(); ++channel) {
    ...
    Sample* intensityChannel =
        getIntensity()->getWrapper().getChannelPointer(channel);
    Sample* modulationIndexChannel =
        getModulationIndex()->getWrapper().getChannelPointer(channel);
    Sample* harmonicsChannel =
        getHarmonics()->getWrapper().getChannelPointer(channel);
    ...
    for (int sample = 0; sample < getNumSamples(); ++sample) {
      ...
      if (a == 1.0 || n < 0.0) {
        ...
        switch (static_cast<int>(mode)) {
          case Mode::ONE_SIDED:
            scale = (a - 1.0) / (a1 - 1.0);
            numerator = sin(t) - a * sin(t - s) -
                        a1 * (sin(t + (n + 1.0) * s) - a * sin(t + n * s));
            break;
          case Mode::TWO_SIDED:
            scale = (a - 1.0) / (a1 + a1 - a - 1.0);
            numerator =
                sin(t) *
                (1.0 - aa - (a1 + a1) * (cos((n + 1.0) * s) - a * cos(n * s)));
            break;
        }
