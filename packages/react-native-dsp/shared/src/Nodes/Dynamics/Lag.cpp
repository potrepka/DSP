...
dsp::Lag::Lag(Type type)
    : Transformer(type, type), lagTime(std::make_shared<Input>(Type::SECONDS)) {
  getInputs().push_back(lagTime);
}

std::shared_ptr<dsp::Input> dsp::Lag::getLagTime() const { return lagTime; }
...
void dsp::Lag::processNoLock() {
  for (size_t channel = 0; channel < getNumChannels(); ++channel) {
    ...
    Sample* lagTimeChannel =
        getLagTime()->getWrapper().getChannelPointer(channel);
    ...
    for (size_t sample = 0; sample < getNumSamples(); ++sample) {
      ...
      state[channel] =
          inputChannel[sample] + pow(0.001, 1.0 / lagSamples) *
