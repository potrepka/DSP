...
dsp::Convolver::Convolver()
    : Transformer(Type::RATIO, Type::RATIO), headSize(0), tailSize(0) {}

size_t dsp::Convolver::getHeadSize() const { return headSize; }
...
size_t dsp::Convolver::getTailSize() const { return tailSize; }
...
std::shared_ptr<dsp::Buffer> dsp::Convolver::getBuffer(
    size_t inChannel, size_t outChannel) const {
  DSP_ASSERT(inChannel < buffers.size() &&
             outChannel < buffers[inChannel].size());
  return buffers[inChannel][outChannel];
}

void dsp::Convolver::setBuffer(size_t inChannel, size_t outChannel,
...
void dsp::Convolver::initConvolvers() {
  ...
  for (size_t inChannel = 0; inChannel < getNumInputChannels(); ++inChannel) {
    for (size_t outChannel = 0; outChannel < getNumInputChannels();
         ++outChannel) {
      initConvolver(inChannel, outChannel);
    }
  }
...
void dsp::Convolver::processNoLock() {
  for (size_t inChannel = 0; inChannel < getNumInputChannels(); ++inChannel) {
    Sample* inputChannel =
        getInput()->getWrapper().getChannelPointer(inChannel);
    for (size_t outChannel = 0; outChannel < getNumOutputChannels();
         ++outChannel) {
      Sample* outputChannel =
          getOutput()->getWrapper().getChannelPointer(outChannel);
      if (convolvers[inChannel][outChannel] != nullptr) {
        for (size_t sample = 0; sample < getNumSamples(); ++sample) {
          input[sample] = inputChannel[sample];
        }
        convolvers[inChannel][outChannel]->process(input.data(), output.data(),
                                                   getNumSamples());
        for (size_t sample = 0; sample < getNumSamples(); ++sample) {
...
void dsp::Convolver::initConvolver(size_t inChannel, size_t outChannel) {
  DSP_ASSERT(inChannel < convolvers.size() &&
             outChannel < convolvers[inChannel].size());
  if (buffers[inChannel][outChannel] != nullptr) {
    ...
    for (size_t sample = 0; sample < getNumSamples(); ++sample) {
      Sample* channel =
          buffers[inChannel][outChannel]->getWrapper().getChannelPointer(0);
      buffer[sample] = channel[sample];
    }
    convolvers[inChannel][outChannel] =
        std::make_unique<fftconvolver::TwoStageFFTConvolver>();
    convolvers[inChannel][outChannel]->init(headSize, tailSize, buffer.data(),
                                            buffer.size());
