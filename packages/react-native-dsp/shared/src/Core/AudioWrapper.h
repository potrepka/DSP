...
public:
  ...
  AudioWrapper& replaceWithApplicationOf(std::function<T(T)> f,
                                         AudioWrapper<T> src);
  AudioWrapper& replaceWithApplicationOf(std::function<T(T, T)> f,
                                         AudioWrapper<T> src1,
                                         AudioWrapper<T> src2);
  ...
  AudioWrapper& replaceWithProductOf(AudioWrapper<T> src1,
                                     AudioWrapper<T> src2);
...
private:
  ...
  AudioWrapper(T* const* data, size_t numChannels, size_t startSample,
               size_t numSamples);
};
