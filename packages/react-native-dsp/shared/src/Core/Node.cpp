...
dsp::Node::~Node() { disconnectAll(); }

bool dsp::Node::isActive() const { return active; }
...
size_t dsp::Node::getNumInputChannels() const { return numInputChannels; }
...
size_t dsp::Node::getNumOutputChannels() const { return numOutputChannels; }
...
size_t dsp::Node::getNumSamples() const { return numSamples; }
...
double dsp::Node::getSampleRate() const { return sampleRate; }
...
double dsp::Node::getOneOverSampleRate() const { return oneOverSampleRate; }

double dsp::Node::getOneOverNumSamples() const { return oneOverNumSamples; }
      ...
      for (const auto& output : node->outputs) {
        for (const auto& input : output->getConnections()) {
          ...
          if (nodeSet.find(child) != nodeSet.end() &&
              explored.find(child) == explored.end()) {
            queue.push(child);
          }
        }
      }
