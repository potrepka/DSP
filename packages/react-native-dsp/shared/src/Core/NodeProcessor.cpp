...
dsp::NodeProcessor::NodeProcessor(size_t numInputChannels,
                                  size_t numOutputChannels, size_t numSamples,
                                  double sampleRate)
    : audioInput(std::make_shared<Output>(Type::RATIO, Space::TIME, 0.0, 0.0,
                                          numInputChannels, numSamples)),
      audioOutput(std::make_shared<Input>(Type::RATIO, Space::TIME, 0.0, 0.0,
                                          numOutputChannels, numSamples)),
      audioInputClipping(std::make_shared<Output>(
          Type::BOOLEAN, Space::TIME, 0.0, 0.0, numInputChannels, numSamples)),
      audioOutputClipping(std::make_shared<Output>(
          Type::BOOLEAN, Space::TIME, 0.0, 0.0, numOutputChannels, numSamples)),
      active(true),
...
dsp::NodeProcessor::~NodeProcessor() { nodes.clear(); }

bool dsp::NodeProcessor::isActive() const { return active; }
...
std::shared_ptr<dsp::Output> dsp::NodeProcessor::getAudioOutputClipping()
    const {
...
size_t dsp::NodeProcessor::getNumSamples() const { return numSamples; }
...
double dsp::NodeProcessor::getSampleRate() const { return sampleRate; }
...
template <typename T>
void dsp::NodeProcessor::process(AudioBuffer<T>& audioBuffer,
  ...
  if (active) {
    ...
    for (size_t channel = 0; channel < audioInput->getNumChannels();
         ++channel) {
    ...
    processClipping(audioOutput->getWrapper(),
                    audioOutputClipping->getWrapper());

    for (size_t channel = 0; channel < audioOutput->getNumChannels();
         ++channel) {
      auto* outputChannel =
          audioOutput->getWrapper().getChannelPointer(channel);
      auto* bufferChannel = audioBuffer.getWritePointer(channel);
      for (auto sample = 0; sample < audioOutput->getNumSamples(); ++sample) {
        bufferChannel[sample] =
            static_cast<T>(clip(outputChannel[sample], -1.0, 1.0));
      }
    }
  } else {
...
template void dsp::NodeProcessor::process(AudioBuffer<float>& audioBuffer,
                                          MidiBuffer& midiBuffer);
template void dsp::NodeProcessor::process(AudioBuffer<double>& audioBuffer,
                                          MidiBuffer& midiBuffer);

void dsp::NodeProcessor::processClipping(Wrapper& audioWrapper,
                                         Wrapper& audioClippingWrapper) {
  for (size_t channel = 0; channel < audioWrapper.getNumChannels(); ++channel) {
    auto* audioChannel = audioWrapper.getChannelPointer(channel);
    auto* audioClippingChannel =
        audioClippingWrapper.getChannelPointer(channel);
