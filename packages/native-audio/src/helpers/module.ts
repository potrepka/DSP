...
/**
 * Emscripten Module interface for native-audio WASM.
...
export async function registerAudioWorkletProcessor(
  ...
  const wasmTextWithoutExport = wasmText.replace(/export\s*\{\s*\w+\s*as\s*default\s*\};?/g, '').replace(/export\s+default\s+\w+;?/g, '')
...
${processorText}
  ...
  const blob = new Blob([workletCode], { type: 'application/javascript;charset=utf-8' })
