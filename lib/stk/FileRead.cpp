...
FileRead ::FileRead() : fd_(0), fileSize_(0), channels_(0), dataType_(0), fileRate_(0.0) {}

FileRead ::FileRead(std::string fileName, bool typeRaw, unsigned int nChannels, StkFormat format, StkFloat rate)
        : fd_(0) {
...
void FileRead ::open(std::string fileName, bool typeRaw, unsigned int nChannels, StkFormat format, StkFloat rate) {
    ...
    if (typeRaw)
        ...
        else if (!strncmp(header, "FORM", 4) && (!strncmp(&header[8], "AIFF", 4) || !strncmp(&header[8], "AIFC", 4)))
            result = getAifInfo(fileName.c_str());
        else {
            if (fseek(fd_, 126, SEEK_SET) == -1)
                goto error;
            if (fread(&header, 2, 1, fd_) != 1)
                goto error;
            if (!strncmp(header, "MI", 2) || !strncmp(header, "IM", 2))
...
bool FileRead ::getRawInfo(const char *fileName, unsigned int nChannels, StkFormat format, StkFloat rate) {
    ...
    if (format == STK_SINT8)
        sampleBytes = 1;
    else if (format == STK_SINT16)
        sampleBytes = 2;
    else if (format == STK_SINT32 || format == STK_FLOAT32)
        sampleBytes = 4;
    else if (format == STK_FLOAT64)
        sampleBytes = 8;
    else {
...
bool FileRead ::getWavInfo(const char *fileName) {
    ...
    if (fread(&chunkSize, 4, 1, fd_) != 1)
        goto error; // Read fmt chunk size.
    ...
    dataType_ = 0;
    if (fseek(fd_, 6, SEEK_CUR) == -1)
        goto error; // Locate bits_per_sample info.
    if (fread(&temp, 2, 1, fd_) != 1)
        goto error;
#ifndef __LITTLE_ENDIAN__
    swap16((unsigned char *)&temp);
    ...
    if (dataType_ == 0) {
        oStream_ << "FileRead: " << temp << " bits per sample with data format " << format_tag << " are not supported ("
                 << fileName << ").";
        return false;
    }
...
bool FileRead ::getSndInfo(const char *fileName) {
    ...
    if (fseek(fd_, 12, SEEK_SET) == -1)
        goto error; // Locate format
    if (fread(&format, 4, 1, fd_) != 1)
        goto error;
#ifdef __LITTLE_ENDIAN__
    ...
    UINT32 offset;
    if (fseek(fd_, 4, SEEK_SET) == -1)
        goto error;
    if (fread(&offset, 4, 1, fd_) != 1)
        goto error;
#ifdef __LITTLE_ENDIAN__
...
bool FileRead ::getAifInfo(const char *fileName) {
    ...
    if (fseek(fd_, 8, SEEK_SET) == -1)
        goto error;
    if (fread(&id, 4, 1, fd_) != 1)
        goto error;
    if (!strncmp(id, "AIFC", 4))
        aifc = true;
    ...
    // Get number of channels from the header.
    SINT16 temp;
    if (fseek(fd_, 4, SEEK_CUR) == -1)
        goto error; // Jump over chunk size
    if (fread(&temp, 2, 1, fd_) != 1)
        goto error;
#ifdef __LITTLE_ENDIAN__
    swap16((unsigned char *)&temp);
    ...
    if (aifc == false) {
        ...
        if (!strncmp(id, "NONE", 4) || !strncmp(id, "sowt", 4)) {
            if (temp <= 8)
                ...
                dataType_ = STK_SINT32;
        } else if ((!strncmp(id, "fl32", 4) || !strncmp(id, "FL32", 4)) && temp == 32)
            dataType_ = STK_FLOAT32;
        else if ((!strncmp(id, "fl64", 4) || !strncmp(id, "FL64", 4)) && temp == 64)
            dataType_ = STK_FLOAT64;
    }
    if (dataType_ == 0) {
...
bool FileRead ::findNextMatArray(SINT32 *chunkSize, SINT32 *rows, SINT32 *columns, SINT32 *nametype) {
    ...
    *chunkSize = 0;
    ...
    if (fseek(fd_, 20, SEEK_CUR) == -1)
        return false;
    SINT32 size;
    if (fread(&size, 4, 1, fd_) != 1)
        return false;
    if (byteswap_)
        swap32((unsigned char *)&size);
    if (size != 8)
        return false;

    // Read dimensions data
    if (fread(rows, 4, 1, fd_) != 1)
        return false;
    if (byteswap_)
        swap32((unsigned char *)rows);
    if (fread(columns, 4, 1, fd_) != 1)
        return false;
    if (byteswap_)
        swap32((unsigned char *)columns);

    // Read array name subelement type
    if (fread(nametype, 4, 1, fd_) != 1)
        return false;
    if (byteswap_)
        swap32((unsigned char *)nametype);
...
bool FileRead ::getMatInfo(const char *fileName) {
    ...
    while (!doneParsing) {
        ...
        if (!haveSampleRate && rows == 1 && columns == 1) { // Parse for sample rate.
            ...
            if (nametype == 1) { // array name > 4 characters
                if (fread(&namesize, 4, 1, fd_) != 1)
                    goto error;
                if (byteswap_)
                    swap32((unsigned char *)&namesize);
                if (namesize != 2)
                    goto tryagain; // expecting name = "fs"
                namesize = 8;      // field must be padded to multiple of 8 bytes
            }
            char name[3];
            name[2] = '\0';
            if (fread(&name, 2, 1, fd_) != 1)
                goto error;
            if (strncmp(name, "fs", 2))
                goto tryagain;

            // Jump to real part data subelement, which is likely to be in a
            // small data format.
            if (fseek(fd_, namesize - 2, SEEK_CUR) == -1)
                goto error;
            UINT32 type;
            StkFloat srate;
            if (fread(&type, 4, 1, fd_) != 1)
                goto error;
            if (byteswap_)
                swap32((unsigned char *)&type);
            if ((type & 0xffff0000) != 0) // small data format
                type = (type & 0x0000ffff);
            else if (fseek(fd_, 4, SEEK_CUR) == -1)
                goto error;
            if (type == 1) { // SINT8
                signed char rate;
                if (fread(&rate, 1, 1, fd_) != 1)
                    goto error;
                srate = (StkFloat)rate;
                ...
                SINT16 rate;
                if (fread(&rate, 2, 1, fd_) != 1)
                    goto error;
                if (byteswap_)
                    swap16((unsigned char *)&rate);
                srate = (StkFloat)rate;
                ...
                FLOAT64 rate;
                if (fread(&rate, 8, 1, fd_) != 1)
                    goto error;
                if (byteswap_)
                    swap64((unsigned char *)&rate);
                srate = (StkFloat)rate;
            } else
                goto tryagain;
            ...
            if (rows < columns) {
                ...
                oStream_ << "FileRead: Transpose the MAT-file array so that audio channels fill matrix rows (not "
                            "columns).";
                return false;
            }
            ...
            if (nametype == 1) { // array name > 4 characters
                ...
                if (fseek(fd_, namesize * 8, SEEK_CUR) == -1)
                    goto error; // jump over array name
            } else {
                if (fseek(fd_, 4, SEEK_CUR) == -1)
                    goto error;
            }
    ...
    tryagain:
        if (haveData && haveSampleRate)
            doneParsing = true;
        else // jump to end of data element and keep trying
                if (fseek(fd_, dataoffset + chunkSize + 8, SEEK_SET) == -1)
            goto error;
    }
