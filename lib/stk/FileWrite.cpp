...
FileWrite ::FileWrite() : fd_(0) {}

FileWrite::FileWrite(std::string fileName, unsigned int nChannels, FILE_TYPE type, Stk::StkFormat format) : fd_(0) {
...
void FileWrite ::open(std::string fileName, unsigned int nChannels, FileWrite::FILE_TYPE type, Stk::StkFormat format) {
    ...
    if (format != STK_SINT8 && format != STK_SINT16 && format != STK_SINT24 && format != STK_SINT32 &&
    ...
    if (fileType_ == FILE_RAW) {
        if (channels_ != 1) {
            oStream_ << "FileWrite::open: STK RAW files are, by definition, always monaural (channels = " << nChannels
                     << " not supported)!";
            handleError(StkError::FUNCTION_ARGUMENT);
        }
...
bool FileWrite ::setWavFile(std::string fileName) {
    ...
    char data[4] = {'d', 'a', 't', 'a'};
    SINT32 dataSize = 0;
    if (fwrite(&hdr, 1, bytesToWrite, fd_) != bytesToWrite)
        goto error;
    if (fwrite(&data, 4, 1, fd_) != 1)
        goto error;
    if (fwrite(&dataSize, 4, 1, fd_) != 1)
        goto error;
...
bool FileWrite ::setSndFile(std::string fileName) {
    ...
    if (fileName.find(".snd") == std::string::npos)
        fileName += ".snd";
    fd_ = fopen(fileName.c_str(), "wb");
    if (!fd_) {
...
bool FileWrite ::setAifFile(std::string fileName) {
    std::string name(fileName);
    if (fileName.find(".aif") == std::string::npos)
        fileName += ".aif";
    fd_ = fopen(fileName.c_str(), "wb");
    if (!fd_) {
    ...
    if (fwrite(&hdr, 4, 5, fd_) != 5)
        goto error;
    if (fwrite(&hdr.nChannels, 2, 1, fd_) != 1)
        goto error;
    if (fwrite(&hdr.sampleFrames, 4, 1, fd_) != 1)
        goto error;
    if (fwrite(&hdr.sampleSize, 2, 1, fd_) != 1)
        goto error;
    if (fwrite(&hdr.srate, 10, 1, fd_) != 1)
        goto error;

    if (dataType_ == STK_FLOAT32) {
        char type[4] = {'f', 'l', '3', '2'};
        char zeroes[2] = {0, 0};
        if (fwrite(&type, 4, 1, fd_) != 1)
            goto error;
        if (fwrite(&zeroes, 2, 1, fd_) != 1)
            goto error;
    } else if (dataType_ == STK_FLOAT64) {
        char type[4] = {'f', 'l', '6', '4'};
        char zeroes[2] = {0, 0};
...
bool FileWrite ::setMatFile(std::string fileName) {
    ...
    strcpy(hdr.heading,
           "MATLAB 5.0 MAT-file, Generated using the Synthesis ToolKit in C++ (STK). By Perry R. Cook and Gary P. "
           "Scavone.");
    for (size_t i = strlen(hdr.heading); i < 124; i++)
        hdr.heading[i] = ' ';
    ...
    *sampleRate = (FLOAT64)Stk::sampleRate();
    ...
    if (fwrite(&hdr, 236, 1, fd_) != 1)
        goto error;

    // Write MATLAB array name
    SINT32 tmp;
    if (namelength > 4) {
        if (fwrite(&namelength, 4, 1, fd_) != 1)
            goto error;
        if (fwrite(name.c_str(), namelength, 1, fd_) != 1)
            goto error;
        tmp = (SINT32)ceil((float)namelength / 8);
        if (fseek(fd_, tmp * 8 - namelength, SEEK_CUR) == -1)
            goto error;
        headsize += tmp * 8;
    } else { // Compressed data element format
        if (fwrite(name.c_str(), namelength, 1, fd_) != 1)
    ...
    if (fseek(fd_, 196, SEEK_SET) == -1)
        goto error;
    if (fwrite(&headsize, 4, 1, fd_) != 1)
        goto error; // Write header size ... will update at end
    if (fseek(fd_, 0, SEEK_END) == -1)
        goto error;
...
void FileWrite ::write(StkFrames &buffer) {
    ...
    if (buffer.channels() != channels_) {
        oStream_ << "FileWrite::write(): number of channels in the StkFrames argument does not match that specified to "
                    "open() function!";
        handleError(StkError::FUNCTION_ARGUMENT);
        return;
    }
