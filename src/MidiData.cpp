...
dsp::MidiData::Iterator::Iterator(std::multimap<int, TimedMidiMessage>::const_iterator it)
        : it(it) {}

dsp::MidiData::Iterator &dsp::MidiData::Iterator::operator++() {
    ++it;
    return *this;
}

bool dsp::MidiData::Iterator::operator==(const Iterator &other) const {
    return it == other.it;
}
...
dsp::TimedMidiMessage dsp::MidiData::Iterator::operator*() const {
    return it->second;
}

dsp::MidiData::MidiData() {}

dsp::MidiData::Iterator dsp::MidiData::begin() const {
    return Iterator(events.begin());
}

dsp::MidiData::Iterator dsp::MidiData::end() const {
    return Iterator(events.end());
}

void dsp::MidiData::addEvent(const MidiMessage &midiMessage, int sample) {
    events.insert({sample, TimedMidiMessage(midiMessage, sample)});
}

void dsp::MidiData::addEvents(const MidiData &midiData, int startSample, int numSamples, int sampleDeltaToAdd) {
