...
std::vector<unsigned int> dsp::Engine::getSampleRates(
    unsigned int inputDevice, unsigned int outputDevice) {
  std::vector<unsigned int> sampleRates;
#ifdef DSP_USE_RTAUDIO
  std::vector<unsigned int> inputSampleRates = getInputSampleRates(inputDevice);
  std::vector<unsigned int> outputSampleRates =
      getOutputSampleRates(outputDevice);
  ...
  std::set_intersection(inputSampleRates.begin(), inputSampleRates.end(),
                        outputSampleRates.begin(), outputSampleRates.end(),
...
unsigned int dsp::Engine::getDefaultSampleRate(unsigned int inputDevice,
...
void dsp::Engine::setup(unsigned int inputDevice, unsigned int outputDevice,
                        unsigned int numSamples, unsigned int sampleRate) {
  lock();
  ...
  RtAudioFormat format =
      (sizeof(Sample) == 8) ? RTAUDIO_FLOAT64 : RTAUDIO_FLOAT32;
  ...
  audioBuffer.setSize(std::max(this->numInputChannels, this->numOutputChannels),
                      this->numSamples);
...
std::string dsp::Engine::getInputDeviceName() const { return inputDeviceName; }
...
unsigned int dsp::Engine::getNumSamples() const { return numSamples; }

unsigned int dsp::Engine::getSampleRate() const { return sampleRate; }

dsp::AudioBuffer<dsp::Sample>& dsp::Engine::getAudioBuffer() {
  return audioBuffer;
}

void dsp::Engine::processAudioBufferNoLock(Sample* inputBuffer,
                                           Sample* outputBuffer) {
  for (unsigned int channel = 0; channel < numInputChannels; ++channel) {
    Sample* audioInputChannel = audioBuffer.getWritePointer(channel);
    for (unsigned int k = 0, sample = channel; k < numSamples;
         ++k, sample += numInputChannels) {
...
#ifdef DSP_USE_RTAUDIO
int dsp::Engine::tick(void* outputBuffer, void* inputBuffer,
                      unsigned int nBufferFrames, double streamTime,
                      RtAudioStreamStatus status, void* pointer) {
  Engine* engine = reinterpret_cast<Engine*>(pointer);
  engine->lock();
  process(reinterpret_cast<Sample*>(inputBuffer),
          reinterpret_cast<Sample*>(outputBuffer), engine);
  engine->unlock();
  return 0;
}
#endif

void dsp::Engine::process(Sample* inputBuffer, Sample* outputBuffer,
                          Engine* engine) {
...
std::vector<unsigned int> dsp::Engine::getInputSampleRates(
