...
template <typename T>
dsp::AudioWrapper<T> dsp::AudioWrapper<T>::getSingleChannel(
...
template <typename T>
dsp::AudioWrapper<T> dsp::AudioWrapper<T>::getSampleRange(
    size_t sampleOffset, size_t numSamples) const {
  DSP_ASSERT(sampleOffset + numSamples <= this->numSamples);
  return AudioWrapper(data, numChannels, startSample + sampleOffset,
                      numSamples);
}
...
dsp::AudioWrapper<T>& dsp::AudioWrapper<T>::replaceWithApplicationOf(
    std::function<T(T)> f, AudioWrapper<T> src) {
...
template <typename T>
dsp::AudioWrapper<T>& dsp::AudioWrapper<T>::replaceWithApplicationOf(
    std::function<T(T, T)> f, AudioWrapper<T> src1, AudioWrapper<T> src2) {
  size_t numChannels =
      std::min({this->numChannels, src1.numChannels, src2.numChannels});
  size_t numSamples =
      std::min({this->numSamples, src1.numSamples, src2.numSamples});
  for (size_t channel = 0; channel < numChannels; ++channel) {
    auto a = data[channel] + startSample;
...
template <typename T>
dsp::AudioWrapper<T>& dsp::AudioWrapper<T>::addProductOf(AudioWrapper<T> src,
                                                         T value) {
  size_t numChannels = std::min({this->numChannels, src.numChannels});
  size_t numSamples = std::min({this->numSamples, src.numSamples});
  for (size_t channel = 0; channel < numChannels; ++channel) {
...
template <typename T>
dsp::AudioWrapper<T>& dsp::AudioWrapper<T>::addProductOf(AudioWrapper<T> src1,
                                                         AudioWrapper<T> src2) {
  size_t numChannels =
      std::min({this->numChannels, src1.numChannels, src2.numChannels});
  size_t numSamples =
...
template <typename T>
dsp::AudioWrapper<T>& dsp::AudioWrapper<T>::replaceWithNegativeOf(
    AudioWrapper<T> src) {
  size_t numChannels = std::min({this->numChannels, src.numChannels});
  size_t numSamples = std::min({this->numSamples, src.numSamples});
  for (size_t channel = 0; channel < numChannels; ++channel) {
...
template <typename T>
dsp::AudioWrapper<T>& dsp::AudioWrapper<T>::replaceWithAbsoluteValueOf(
    AudioWrapper<T> src) {
  size_t numChannels = std::min({this->numChannels, src.numChannels});
  size_t numSamples = std::min({this->numSamples, src.numSamples});
  for (size_t channel = 0; channel < numChannels; ++channel) {
...
template <typename T>
dsp::AudioWrapper<T>& dsp::AudioWrapper<T>::replaceWithSumOf(
    AudioWrapper<T> src, T value) {
  size_t numChannels = std::min({this->numChannels, src.numChannels});
  size_t numSamples = std::min({this->numSamples, src.numSamples});
  for (size_t channel = 0; channel < numChannels; ++channel) {
    auto a = data[channel] + startSample;
...
template <typename T>
dsp::AudioWrapper<T>& dsp::AudioWrapper<T>::replaceWithSumOf(
    AudioWrapper<T> src1, AudioWrapper<T> src2) {
  size_t numChannels =
      std::min({this->numChannels, src1.numChannels, src2.numChannels});
  size_t numSamples =
      std::min({this->numSamples, src1.numSamples, src2.numSamples});
...
template <typename T>
dsp::AudioWrapper<T>& dsp::AudioWrapper<T>::replaceWithProductOf(
    AudioWrapper<T> src, T value) {
  size_t numChannels = std::min({this->numChannels, src.numChannels});
  size_t numSamples = std::min({this->numSamples, src.numSamples});
  for (size_t channel = 0; channel < numChannels; ++channel) {
...
template <typename T>
dsp::AudioWrapper<T>& dsp::AudioWrapper<T>::replaceWithMinOf(
    AudioWrapper<T> src, T value) {
  size_t numChannels = std::min({this->numChannels, src.numChannels});
  size_t numSamples = std::min({this->numSamples, src.numSamples});
  for (size_t channel = 0; channel < numChannels; ++channel) {
...
template <typename T>
dsp::AudioWrapper<T>& dsp::AudioWrapper<T>::replaceWithMaxOf(
    AudioWrapper<T> src, T value) {
  size_t numChannels = std::min({this->numChannels, src.numChannels});
  size_t numSamples = std::min({this->numSamples, src.numSamples});
  for (size_t channel = 0; channel < numChannels; ++channel) {
...
template <typename T>
void dsp::AudioWrapper<T>::setSample(size_t channel, size_t sampleOffset,
                                     T value) const {
  DSP_ASSERT(channel < numChannels);
  DSP_ASSERT(sampleOffset < numSamples);
...
template <typename T>
dsp::AudioWrapper<T>::AudioWrapper(T* const* data, size_t numChannels,
                                   size_t numSamples)
