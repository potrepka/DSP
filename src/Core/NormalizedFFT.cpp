...
dsp::NormalizedFFT::NormalizedFFT()
        : size(0)
        , oneOverSize(0.0) {}
...
std::size_t dsp::NormalizedFFT::getSize() {
    return size;
}
...
void dsp::NormalizedFFT::toRealImaginary(Sample *timeChannel, Sample *realChannel, Sample *imaginaryChannel) {
...
void dsp::NormalizedFFT::fromRealImaginary(Sample *realChannel, Sample *imaginaryChannel, Sample *timeChannel) {
    for (size_t k = 0; k < size; ++k) {
...
void dsp::NormalizedFFT::toMagnitudePhase(Sample *timeChannel, Sample *magnitudeChannel, Sample *phaseChannel) {
    for (size_t k = 0; k < size; ++k) {
        time[k] = timeChannel[k] * oneOverSize;
    }
    audioFFT.fft(time.data(), real.data(), imaginary.data());
...
void dsp::NormalizedFFT::fromMagnitudePhase(Sample *magnitudeChannel, Sample *phaseChannel, Sample *timeChannel) {
    for (size_t k = 0; k < getComplexSize(); ++k) {
