...
dsp::Type dsp::Buffer::getType() const { return type; }
...
dsp::Sample dsp::Buffer::getRange() const { return range; }
...
dsp::Sample dsp::Buffer::getDefaultValue() const { return defaultValue; }
...
size_t dsp::Buffer::getNumChannels() const { return data.getNumChannels(); }
...
size_t dsp::Buffer::getNumSamples() const { return data.getNumSamples(); }
...
dsp::Array dsp::Buffer::getChannelValues() const { return channelValues; }
...
dsp::Data& dsp::Buffer::getData() { return data; }

dsp::Wrapper& dsp::Buffer::getWrapper() { return wrapper; }
...
dsp::Input::Mode dsp::Input::getMode() const { return mode; }
  ...
  if (connections.size() == 0) {
    ...
    switch (mode) {
      case Mode::SUM:
        ...
        break;
      case Mode::MINIMUM:
        wrapper.fill(std::numeric_limits<Sample>::infinity());
        break;
      case Mode::MAXIMUM:
        wrapper.fill(-std::numeric_limits<Sample>::infinity());
        break;
...
void dsp::Input::processNoLock() {
  for (const auto& output : connections) {
    switch (mode) {
      case Mode::SUM:
        wrapper.add(output->getWrapper());
        break;
      case Mode::MINIMUM:
        wrapper.replaceWithMinOf(wrapper, output->getWrapper());
        break;
      case Mode::MAXIMUM:
        wrapper.replaceWithMaxOf(wrapper, output->getWrapper());
        break;
    }
  }
  switch (type) {
...
void dsp::Input::addConnection(std::shared_ptr<Output> output) {
  if (std::find(connections.begin(), connections.end(), output) ==
      connections.end()) {
...
void dsp::Input::removeConnection(std::shared_ptr<Output> output) {
  connections.erase(std::remove(connections.begin(), connections.end(), output),
                    connections.end());
}
...
void dsp::Output::prepareNoLock() { fillChannels(); }
...
void dsp::Output::addConnection(std::shared_ptr<Input> input) {
  if (std::find(connections.begin(), connections.end(), input) ==
      connections.end()) {
...
void dsp::Output::removeConnection(std::shared_ptr<Input> input) {
  connections.erase(std::remove(connections.begin(), connections.end(), input),
                    connections.end());
}
...
void dsp::operator>>(std::shared_ptr<Output> output,
