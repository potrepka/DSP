...
bool dsp::MidiMessage::isNoteOff() const { return (bytes[0] & 0xf0) == 0x80; }

bool dsp::MidiMessage::isNoteOn() const { return (bytes[0] & 0xf0) == 0x90; }
...
bool dsp::MidiMessage::isMidiClock() const { return bytes[0] == 0xf8; }

bool dsp::MidiMessage::isMidiStart() const { return bytes[0] == 0xfa; }

bool dsp::MidiMessage::isMidiContinue() const { return bytes[0] == 0xfb; }

bool dsp::MidiMessage::isMidiStop() const { return bytes[0] == 0xfc; }
...
dsp::MidiMessage dsp::MidiMessage::noteOff(uint8_t channel, uint8_t noteNumber,
                                           uint8_t velocity) {
...
dsp::MidiMessage dsp::MidiMessage::noteOn(uint8_t channel, uint8_t noteNumber,
                                          uint8_t velocity) {
  DSP_ASSERT(channel >= 1 && channel <= 16);
  DSP_ASSERT(noteNumber < 128);
  DSP_ASSERT(velocity < 128);
...
dsp::MidiMessage dsp::MidiMessage::programChange(uint8_t channel,
...
dsp::MidiMessage dsp::MidiMessage::pitchWheel(uint8_t channel,
                                              int pitchWheelValue) {
  DSP_ASSERT(channel >= 1 && channel <= 16);
  return MidiMessage(0xe0 | (channel - 1), pitchWheelValue & 127,
                     (pitchWheelValue >> 7) & 127);
}
...
dsp::MidiMessage dsp::MidiMessage::songPositionPointer(
    int songPositionPointerMidiBeat) {
  return MidiMessage(0xf2, songPositionPointerMidiBeat & 127,
                     (songPositionPointerMidiBeat >> 7) & 127);
}

dsp::MidiMessage dsp::MidiMessage::midiClock() { return MidiMessage(0xf8); }

dsp::MidiMessage dsp::MidiMessage::midiStart() { return MidiMessage(0xfa); }

dsp::MidiMessage dsp::MidiMessage::midiContinue() { return MidiMessage(0xfb); }

dsp::MidiMessage dsp::MidiMessage::midiStop() { return MidiMessage(0xfc); }

std::vector<uint8_t>& dsp::MidiMessage::getBytes() { return bytes; }

const uint8_t* dsp::MidiMessage::getRawData() const { return bytes.data(); }

size_t dsp::MidiMessage::getRawDataSize() const { return bytes.size(); }

dsp::TimedMidiMessage::TimedMidiMessage(MidiMessage midiMessage,
                                        size_t samplePosition)
    : samplePosition(samplePosition), midiMessage(midiMessage) {}
