...
void dsp::MidiProcessor::Input::callback(
  ...
  input->messages.insert(
      {input->messageTime += delta,
       MidiMessage(bytesPointer->data(), bytesPointer->size())});
  input->unlock();
}

dsp::MidiProcessor::Input::Input(unsigned int port) : messageTime(0.0) {
...
std::multimap<double, dsp::MidiMessage>&
dsp::MidiProcessor::Input::getMessages() {
  return messages;
}

dsp::MidiProcessor::Output::Output(unsigned int port) { setPort(port); }
...
void dsp::MidiProcessor::Output::sendMessageWithDelay(
    std::vector<unsigned char> bytes, int64_t nanoseconds) {
#ifdef DSP_USE_RTMIDI
...
size_t dsp::MidiProcessor::getNumSamples() const { return numSamples; }
...
double dsp::MidiProcessor::getSampleRate() const { return sampleRate; }
...
std::vector<std::shared_ptr<dsp::MidiProcessor::Input>>&
dsp::MidiProcessor::getInputs() {
  return inputs;
}

std::vector<std::shared_ptr<dsp::MidiProcessor::Output>>&
dsp::MidiProcessor::getOutputs() {
  return outputs;
}

dsp::MidiBuffer& dsp::MidiProcessor::getMidiBuffer() { return midiBuffer; }
...
void dsp::MidiProcessor::processInputs() {
  ...
  for (const auto& input : inputs) {
    ...
    for (const auto& pair : input->getMessages()) {
      ...
      size_t timeInBuffer =
          static_cast<size_t>((pair.first - messageTime) * sampleRate);
      if (timeInBuffer < 0) {
        timeInBuffer = 0;
      }
...
void dsp::MidiProcessor::processOutputs() {
  ...
  for (const auto& output : outputs) {
    ...
    for (const auto meta : midiBuffer) {
      ...
      output->sendMessageWithDelay(bytes,
                                   meta.samplePosition * oneOverSampleRate);
    }
    output->unlock();
  }
