...
template <typename T>
dsp::AudioWrapper<T>::AudioWrapper(AudioBuffer<T> &audioData)
...
template <typename T>
dsp::AudioWrapper<T> dsp::AudioWrapper<T>::getSingleChannelBlock(size_t channel) const {
    assert(channel < numChannels);
    return AudioBlock(data + channel, 1, startSample, numSamples);
}

template <typename T>
dsp::AudioWrapper<T> dsp::AudioWrapper<T>::getSubBlock(size_t sampleOffset, size_t numSamples) const {
