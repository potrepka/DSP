...
dsp::FrequencyToNote::FrequencyToNote(Domain domain)
    : Transformer(Type::HERTZ, Type::RATIO, domain),
...
void dsp::FrequencyToNote::processNoLock() {
  getOutput()->getWrapper().replaceWithApplicationOf(
      [](Sample x, Sample y) { return y == 0.0 ? 0.0 : log2(x / y); },
      getInput()->getWrapper(), getTuningFrequency()->getWrapper());
