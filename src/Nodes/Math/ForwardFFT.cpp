...
dsp::ForwardFFT::ForwardFFT()
    : Consumer(Type::RATIO),
      magnitude(std::make_shared<Output>(Type::RATIO, Domain::FREQUENCY)),
      phase(std::make_shared<Output>(Type::RATIO, Domain::FREQUENCY)) {
...
std::shared_ptr<dsp::Output> dsp::ForwardFFT::getPhase() const { return phase; }
...
void dsp::ForwardFFT::processNoLock() {
  for (size_t channel = 0; channel < getNumChannels(); ++channel) {
    ...
    Sample* magnitudeChannel =
        getMagnitude()->getWrapper().getChannelPointer(channel);
