...
void dsp::Sequencer::processNoLock() {
  if (sequences.size() > 0) {
    ...
    for (size_t channel = 0; channel < getNumChannels(); ++channel) {
      Sample* sequenceIndexChannel =
          getSequenceIndex()->getWrapper().getChannelPointer(channel);
      Sample* positionIndexChannel =
          getPositionIndex()->getWrapper().getChannelPointer(channel);
      ...
      for (size_t sample = 0; sample < getNumSamples(); ++sample) {
        ...
        if (p < sequences.size() && sequences[p] != nullptr) {
          ...
          if (numChannels > 0 && numSamples > 0) {
            Sample* sequenceChannel =
                sequences[p]->getWrapper().getChannelPointer(channel %
                                                             numChannels);
            size_t index =
                static_cast<size_t>(positionIndexChannel[sample]) % numSamples;
            outputChannel[sample] =
                sequenceChannel[index + (index < 0) * numSamples];
          } else {
