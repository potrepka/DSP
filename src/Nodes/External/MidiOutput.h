...
public:
  MidiOutput(std::shared_ptr<MidiBuffer> outputMessages,
  ...
  std::function<void()> processNoteOn(uint8 channel,
                                      std::unordered_set<uint8> noteSet);
  std::function<void()> processNoteOff(uint8 channel,
                                       std::unordered_set<uint8> noteSet);
  std::function<void()> processNotePressure(uint8 channel,
                                            std::unordered_set<uint8> noteSet);
  std::function<void()> processControl(uint8 channel);
  std::function<void()> processControlValue(
...
protected:
  void processImpulse(std::unordered_set<uint8> itemSet,
  ...
  void processContinuous(std::unordered_set<uint8> itemSet,
                         std::function<MidiMessage(uint8, Sample)> publish);
