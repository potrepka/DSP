...
std::shared_ptr<dsp::Input> dsp::CompressorGate::getSoftness() const {
  return softness;
}
...
dsp::Sample dsp::CompressorGate::getGainResponse(size_t channel,
                                                 Sample control) {
  lock();
  ...
  if (getNumSamples() > 0) {
    ...
    const Sample softness =
        getSoftness()->getWrapper().getSample(channel, lastSample);
    ...
    unlock();
    return control * getGainResponse(control, threshold, softness,
                                     compressionRatio, gateRatio);
  } else {
    unlock();
    return control;
  }
...
void dsp::CompressorGate::processNoLock() {
  for (size_t channel = 0; channel < getNumChannels(); ++channel) {
    ...
    Sample* controlChannel =
        getControl()->getWrapper().getChannelPointer(channel);
    Sample* thresholdChannel =
        getThreshold()->getWrapper().getChannelPointer(channel);
    Sample* softnessChannel =
        getSoftness()->getWrapper().getChannelPointer(channel);
    Sample* compressionRatioChannel =
        getCompressionRatio()->getWrapper().getChannelPointer(channel);
    Sample* gateRatioChannel =
        getGateRatio()->getWrapper().getChannelPointer(channel);
    ...
    for (size_t sample = 0; sample < getNumSamples(); ++sample) {
      ...
      const Sample gainTarget = getGainResponse(control, threshold, softness,
                                                compressionRatio, gateRatio);
      Sample decay;
      ...
      state[channel] =
          gainTarget + pow(0.001, 1.0 / decay) * (state[channel] - gainTarget);
      gain = state[channel];
      output = input * gain;
    }
  }
...
dsp::Sample dsp::CompressorGate::getGainResponse(const Sample& control,
                                                 ...
                                                 const Sample& compressionRatio,
                                                 const Sample& gateRatio) {
  const Sample controlMinusThreshold = log(abs(control)) - log(abs(threshold));
  if (isinf(controlMinusThreshold)) {
    return 1.0;
  }
  Sample logGain;
  const Sample halfKnee = abs(softness) * LOG_OF_TWO;
  const Sample ratioDifference = 1.0 / compressionRatio - gateRatio;
  if (controlMinusThreshold > -halfKnee && controlMinusThreshold < halfKnee &&
      ratioDifference != 0.0) {
    ...
    logGain = t * (t * halfKnee * ratioDifference + kneeTimesGR) -
              0.5 * kneeTimesGR - controlMinusThreshold;
  } else {
    const Sample ratioMinusOne = controlMinusThreshold > 0.0
