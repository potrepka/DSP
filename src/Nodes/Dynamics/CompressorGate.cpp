...
dsp::Sample dsp::CompressorGate::getGainResponse(size_t channel, Sample control) {
    ...
    if (getNumSamples() > 0) {
        ...
        const Sample softness = getSoftness()->getWrapper().getSample(channel, lastSample);
        ...
        unlock();
        return control * getGainResponse(control, threshold, softness, compressionRatio, gateRatio);
    } else {
        unlock();
        return control;
    }
...
void dsp::CompressorGate::processNoLock() {
    for (size_t channel = 0; channel < getNumChannels(); ++channel) {
        ...
        Sample *softnessChannel = getSoftness()->getWrapper().getChannelPointer(channel);
