...
std::shared_ptr<dsp::Input> dsp::Clipper::getMinimum() const { return minimum; }

std::shared_ptr<dsp::Input> dsp::Clipper::getMaximum() const { return maximum; }

void dsp::Clipper::processNoLock() {
  for (size_t channel = 0; channel < getNumChannels(); ++channel) {
    Sample* inputChannel = getInput()->getWrapper().getChannelPointer(channel);
    Sample* minChannel = getMinimum()->getWrapper().getChannelPointer(channel);
    Sample* maxChannel = getMaximum()->getWrapper().getChannelPointer(channel);
