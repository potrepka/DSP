...
std::shared_ptr<dsp::Input> dsp::Clipper::getMin() const {
    return min;
}

std::shared_ptr<dsp::Input> dsp::Clipper::getMax() const {
    return max;
}
    ...
    for (size_t channel = 0; channel < getNumChannels(); ++channel) {
        ...
        for (size_t sample = 0; sample < getNumSamples(); ++sample) {
            ...
            switch (static_cast<int>(mode)) {
                case Mode::CLIP: output = clip(input, min, max); break;
                case Mode::WRAP: output = wrap(input - min, max - min) + min; break;
                case Mode::FOLD: {
